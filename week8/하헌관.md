...
```
in 과 hasOwnProperty()가 [[Prototype]] 연쇄의 확인에 따라 차이가 있는 반면, Object.keys()와 Object.getPropertyName()은 모두 주어진 객체만 확인함
...
이런 기법을 더형성(Polymorphism) 또는 가상 다형성, 상대적 다형성이라고 함
...
```
Car에는 Vehicle에서 복하나 프로퍼티와 함수 사본이 있음  
